...
# f_WT=2 = filtro remoto; sortBy=DD = mais recentes primeiro.
...
# - campo de busca: input keywords na página de jobs/search
# - lista de vagas: cards da listagem (título + empresa + local/modelo)
# - etiqueta de modelo: texto "Remoto" no card ou na página da vaga
# - paginação/sort: sortBy=DD já ordena por data (mais recentes primeiro)
...
site_buscar_termos() {
  ...
  awk '/^[[:space:]]*-[[:space:]]*linkedin:/{p=1; print; next} p&&/^[[:space:]]*-[[:space:]]*[a-z]+:/{p=0} p' "$prompt" 2>/dev/null
